...
live Q&A), montage, overview with no Q&A portion, PSA, interview reels \
with no audible questions → produced_content
...
- "produced_content": A pre-recorded, edited, or narrated video with NO \
audible Q&A. This includes: (a) montages, explainers, PSAs, narrated \
documentaries, ham radio knowledge episodes; (b) interview reels where \
the interviewer's audio has been edited out so only the astronaut is heard. \
If there are no audible questions in the transcript, it is produced_content.
- "other": Anything that doesn't fit the above (ceremony, lecture, etc).

IMPORTANT RULES:
1. If the transcript has ONLY 1 speaker and no audible questions from anyone else, \
classify as "produced_content" — even if the filename says "Interview" or "CCP_Interview". \
A single speaker means whatever questions were asked are NOT in the audio.
2. If the transcript shows a conversational back-and-forth between a TV host and \
an astronaut (2+ speakers), classify as "media_interview" even if speaker count \
is >=3 (audio bleed from broadcast can create extra speaker labels).
...
if there is any audible Q&A pattern visible.
...
highlight reels.
6. If there are 2+ speakers with audible questions and answers, it is NOT \
produced_content — classify based on the Q&A pattern (student_qa, press_conference, \
media_interview, panel, etc.).
